...
В начале статьи я писал, что BF можно использовать для проверки конфигурационных изменений перед внесением продакшн. Теперь я предлагаю рассмотреть процесс проведения тестирования сети на базе **RobotFramework**. Для этого я написал небольшой модуль на основе PyBatfish, позволяющий выполнять следующие проверки:
